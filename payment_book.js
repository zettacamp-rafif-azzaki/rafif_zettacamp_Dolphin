...
function Purchase_Book(book, discount, tax, stock, purchase, creditDuration){
    ...
    if(isOnLibrary){
        ...
        return_object.push(CalculateTerm(creditDuration, priceTaxDiscount));
...
async function CalculateTerm(creditDuration, priceTaxDiscount){
    ...
    priceArray.forEach((value, index) => {
        if(index != 0){
        // console.log(priceArray[index - 1] + " - " + priceArray[index] + " = ");
        // console.log(+priceArray[index - 1] - +priceArray[index]);

        howMuchPay.push(+priceArray[index - 1] - +priceArray[index]);
        }
    ...
    dateArray.map((i, index) => {
        ...
        dateArray[index] = {"dueDate" : string, "dueCredit" : priceArray[index], "pay": howMuchPay[index]};
